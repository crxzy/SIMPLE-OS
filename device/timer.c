...
static void frequency_set(uint8_t counter_port, uint8_t counter_no, uint8_t rwl,
    ...
    /*
     * 2018年8月19日22点59分
     * 书中为  outb(counter_port, (uint8_t)counter_value >> 8);
     * 此乃 BUG！
     */
...
static void intr_timer_handler(void) {
    ...
    ASSERT(cur_thread->stack_magic == 0x19970119); // 检查栈是否溢出

    cur_thread->elapsed_ticks++; // 此线程占用的cpu时间嘀
    ticks++;                     //总嘀哒数
...
void timer_init() {
    ...
    put_str(" register_handler 0X20\n");
    // 设置8253的定时周期
    frequency_set(COUNTRER0_PORT, COUNTER0_NO, READ_WRITE_LATCH, COUNTER_MODE,
                  COUNTER0_VALUE);
