 ...
 * 一个页框大小的位图可表示128M内存（4096*8*4096/1024/1024）, 位图位置安排在地址0xc009a000,
 * 4个页框的位图,可表示512M内存 */
#define MEM_BITMAP_BASE 0xc009a000
/*************************************/

/* 0xc0000000内核从虚拟地址3G起. 0x100000跨过低端1M内存 */
...
struct virtual_addr kernel_vaddr;   // 内核分配虚拟地址结构

/* 在pf表示的虚拟内存池中申请pg_cnt个虚拟页 */
static void *vaddr_get(enum pool_flags pf, uint32_t pg_cnt) {
    int vaddr_start = 0, bit_idx_start = -1;
    struct virtual_addr *vaddr_kou;
    uint32_t cnt = 0;
    if (pf == PF_KERNEL) {
        vaddr_kou = &kernel_vaddr;
    } else if (pf == PF_USER) {
        vaddr_kou = &running_thread()->userprog_vaddr;
    }
    bit_idx_start = bitmap_scan(&vaddr_kou->vaddr_bitmap, pg_cnt);
    if (bit_idx_start == -1) {
        return NULL;
    }
    while (cnt < pg_cnt) {
        bitmap_set(&vaddr_kou->vaddr_bitmap, bit_idx_start + cnt++, 1);
    }
    vaddr_start = vaddr_kou->vaddr_start + bit_idx_start * PG_SIZE;
...
void *malloc_page(enum pool_flags pf, uint32_t pg_cnt) {
    ...
    ***********************************************************/
    ...
    while (cnt-- > 0) {
        ...
        if (page_phyaddr == NULL) { // 失败时要将曾经已申请的虚拟地址和物理页全部回滚. 未完成
            return NULL;
        }
...
static void mem_pool_init(uint32_t all_mem) {
    ...
    uint16_t all_free_pages = free_mem / PG_SIZE; // 1页为4k
    ...
    uint32_t up_start = used_mem + kernel_free_pages * PG_SIZE; // User Pool start,用户内存池的起始地址
    ...
    kernel_vaddr.vaddr_bitmap.btmp_bytes_len = kbm_length; // 内核堆的虚拟地址,和内核内存池大小一致
...
void mem_init() {
    ...
    // 0x900 loader 加载首地址， MBR加载LOADER时为LOADER大小
